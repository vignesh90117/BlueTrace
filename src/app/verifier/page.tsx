...
export default function VerifierWorkbenchPage() {
  ...
  const [selectedProjectId, setSelectedProjectId] = useState<string>(
    projects.find(p => p.status === 'field_approved')?.id || projects[0]?.id
  );
  
  const [verifierNotes, setVerifierNotes] = useState(
    'Audited Field Officer ground-truth inspection report. Soil core chromatography and Sentinel-2 L2A canopy indices cross-checked against IPCC VM0033 allometric models. Authorized for on-chain credit minting.'
  );
  ...
  const handleApproveAndMint = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedProject || !selectedProject.mrvResult) return;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      {proj.status === 'credits_issued' ? (
                        ...
                        <span className="text-[10px] font-semibold text-slate-400 bg-slate-800 px-2 py-0.5 rounded-full flex items-center gap-1">
                          <Clock className="w-3 h-3" /> In Stage 2 Field Review
                        </span>
                      )}
                    </div>
          ...
          <div className="lg:col-span-8 space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                {selectedProject.fieldInspection ? (
                  ...
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold bg-amber-500/20 text-amber-300 border border-amber-500/40 flex items-center gap-1">
                    <Clock className="w-3 h-3" /> Pending Field Review
                  </span>
                )}
              </div>
            ...
            {selectedProject.fieldInspection ? (
              ...
              <p className="text-xs text-slate-400">
                This project has not yet been stamped by the Field Officer. You can review and approve it after Stage 2 is signed.
              </p>
            )}
          </div>
          ...
          <form onSubmit={handleApproveAndMint} className="p-8 rounded-3xl glass-panel border border-amber-500/30 space-y-5 shadow-2xl">
            ...
            {signSuccessTx && (
              <div className="p-4 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 text-xs space-y-1.5 animate-in zoom-in-95 duration-150">
                <div className="font-bold flex items-center gap-1.5">
                  <CheckCircle2 className="w-4 h-4 text-emerald-400" /> Final Verification Anchored & Credits Minted!
                </div>
            ...
            <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 font-mono text-xs text-slate-400 space-y-1.5">
              <div className="flex justify-between">
                ...
                <span className="text-teal-400 font-bold">
                  {(selectedProject.mrvResult?.reportSha256Hash || '').slice(0, 18) + '...'}
                </span>
              </div>
              <div className="flex justify-between">
                <span>PDF Serial Number Format:</span>
                <span className="text-purple-300 font-bold">
                  {`${selectedProject.id}-2026-B1-0001-${Math.round(selectedProject.mrvResult?.netIssuableCreditsTCO2e || 0)}`}
                </span>
              </div>
              <div className="flex justify-between">
                <span>Credits to Mint:</span>
                <span className="text-emerald-400 font-bold text-sm">
                  {(selectedProject.mrvResult?.netIssuableCreditsTCO2e || 0).toLocaleString()} BCT (tCO2e)
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
              ...
              <button
                type="submit"
                disabled={isSigning || !selectedProject.mrvResult || selectedProject.status === 'credits_issued'}
                className="w-full sm:w-auto px-6 py-3 rounded-xl bg-gradient-to-r from-amber-500 via-amber-600 to-teal-500 hover:from-amber-600 hover:to-teal-600 text-slate-950 font-bold text-xs shadow-lg shadow-amber-500/20 flex items-center justify-center gap-2 transition-all hover:scale-105 disabled:opacity-50"
              >
                {isSigning ? (
                  <>
                  ...
                  <>
                    ...
                    <span>{selectedProject.status === 'credits_issued' ? 'Credits Already Minted' : 'Sign On-Chain & Mint Credits'}</span>
                  </>
                )}
              </button>
            </div>
